...
static int S9xCompareSDD1IndexEntries(const void *p1,
                                      const void *p2) {
...
void S9xParseArg(char **argv,
                 int &index,
                 int argc) {
}

bool8 S9xOpenSnapshotFile(const char *fname,
                          bool8 read_only,
                          STREAM*file) {
    if (read_only) {
...
bool8_32 S9xDeinitUpdate(int Width,
                         int Height,
                         bool8_32) {
    if (mInMenu) return TRUE;
...
bool8 S9xReadMousePosition(int /* which1 */,
                           int &/* x */,
                           int & /* y */,
                           uint32 & /* buttons */) {
...
bool8 S9xReadSuperScopePosition(int & /* x */,
                                int & /* y */,
                                uint32 & /* buttons */) {
...
void S9xSaveSRAM(int showWarning) {
    if (CPU.SRAMModified) {
        if (!Memory.SaveSRAM((s8 *) S9xGetFilename(".srm"))) {
            MenuMessageBox("Saving SRAM", "Failed!", "",
                           MENU_MESSAGE_BOX_MODE_PAUSE);
        }
    } else if (showWarning) {
        MenuMessageBox("SRAM saving ignored",
                       "No changes have been made to SRAM", "",
                       MENU_MESSAGE_BOX_MODE_MSG);
    }
}
...
bool8_32 S9xOpenSoundDevice(int a,
                            unsigned char b,
                            int c) {
...
int Run(int sound) {
    ...
    Settings.SkipFrames = mMenuOptions.frameSkip == 0 ? AUTO_FRAMERATE :
        ...
        */
        ...
        if (LastAudioRate != mMenuOptions.soundRate ||
            ...
            S9xInitSound(mMenuOptions.soundRate,
                         mMenuOptions.stereo, sal_AudioGetSamplesPerFrame() *
                                              sal_AudioGetBytesPerSample());
    ...
    while (!mEnterMenu) {
        ...
        if (SamplesDoneThisFrame < sal_AudioGetSamplesPerFrame())
            sal_AudioGenerate(
                    sal_AudioGetSamplesPerFrame() - SamplesDoneThisFrame);
...
int SnesRomLoad() {
    ...
    if (!Memory.LoadROM(mRomName)) {
        MenuMessageBox("Loading ROM", mRomName, "Failed!",
                       MENU_MESSAGE_BOX_MODE_PAUSE);
        return SAL_ERROR;
    }
    MenuMessageBox("Done loading the ROM", mRomName, "",
                   MENU_MESSAGE_BOX_MODE_MSG);
...
int mainEntry(int argc,
    ...
    while (1) {
        ...
        if (event == EVENT_LOAD_ROM) {
            if (mRomName[0] != 0) {
                MenuMessageBox("Saving SRAM...", "", "",
                               MENU_MESSAGE_BOX_MODE_MSG);
                PSNESForceSaveSRAM();
            }
            if (SnesRomLoad() == SAL_ERROR) {
                mRomName[0] = 0;
                MenuMessageBox("Failed to load ROM", mRomName,
                               "Press any button to continue",
                               MENU_MESSAGE_BOX_MODE_PAUSE);
                sal_Reset();
                return 0;
            } else {
                MenuMessageBox("Loading game settings...", mRomName, "",
                               MENU_MESSAGE_BOX_MODE_MSG);
                LoadCurrentOptions();
